...
FROM
    ...
    (
        -- 1. Tentativa Clássica (Exata)
        HIK.name COLLATE Latin1_General_CI_AI = USU.NOMEUSU
        OR HIK.name COLLATE Latin1_General_CI_AI = USU.NOMEUSUCPLT

        -- 2. Tentativa pelo LOGIN com Curinga (A que vai salvar você!)
        -- Transforma VITOR.OLIVEIRA em VITOR%OLIVEIRA
        -- Isso vai casar com "VITOR GABRIEL... L. OLIVEIRA"
        OR HIK.name COLLATE Latin1_General_CI_AI LIKE REPLACE(USU.NOMEUSU, '.', '%')

        -- 3. Tentativa inversa: Nome do Portão (Abreviado) dentro do Nome Completo
        -- Transforma "RESENDE L. OLIVEIRA" em "RESENDE L% OLIVEIRA"
        -- Isso vai casar com "RESENDE LOPES OLIVEIRA"
